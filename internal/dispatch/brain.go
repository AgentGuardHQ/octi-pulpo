...
type Constraint struct {
	Type        string // "all_drivers_down", "p0_bugs", "p0_prs_ready", "stale_prs", "none"
	Description string
...
//   - Chain monitoring: detect stalled chains
...
//   - Self-heal: detect stuck agents (triage flag), alert CTO
//
// The squad-era SR/EM/QA per-repo senior dispatch layer was excised in
// octi#271 Phase 2+3. Work now flows through task adapters only — Clawta,
// GH Actions (Copilot), Anthropic, OpenClaw.
type Brain struct {
	...
	// Self-heal dedup: tracks when each agent was last alerted to avoid
	// flooding ntfy on every tick. Keys are agent names.
...
func (b *Brain) Tick(ctx context.Context) {
	...
	// 5. Self-heal: stuck agents
	b.maybeSelfHeal(ctx)

	// 5b. Progress gap detection: warn on active claims with no snapshots for >2 min
	b.checkProgressGaps(ctx)

	// 5c. Swarm assembly line: queue-aware dispatch with stagger + time-gating
...
func (b *Brain) maybeRunSwarmCycle(ctx context.Context) {
	...
	// would actually be dispatchable right now.
...
// how long to suppress re-dispatch.
...
// maybeSelfHeal runs adaptive recovery checks on every tick. Currently only
// detects stuck agents (TriageFlag=true) and alerts the notifier.
...
// a progress snapshot in the last 2 minutes.
...
// a one-time alert (per 12h window) for each.
	...
	for _, p := range profiles {
		...
		if b.notifier != nil && b.notifier.Enabled() {
			if err := b.notifier.PostStuckAgentAlert(ctx, p.Name, p.ConsecutiveFails); err != nil {
				b.log.Printf("self-heal: alert for %s: %v", p.Name, err)
			}
		}
	}
}

// maybeNotifyConstraintChange fires edge-triggered alerts when driver
...
// After the squad excision (octi#271), dispatch only runs through adapters,
// so leverage actions always carry a Task for adapter routing — agent-name
// queue fallback is gone.
...
// leverageForP0 picks the top P0 sprint item for adapter dispatch.
...
// leverageForNextSprint picks the next dispatchable sprint item for adapter dispatch.
...
// executeLeverageAction dispatches the chosen action through a task adapter.
// After the squad excision (octi#271), agent-name queue fallback was removed —
// dispatch only runs when an adapter accepts the task.
func (b *Brain) executeLeverageAction(ctx context.Context, action LeverageAction) {
	if os.Getenv("OCTI_BRAIN_DISPATCH") == "0" {
		b.log.Printf("leverage: BLOCKED (dispatch disabled, score=%.1f, reason=%s)", action.Score, action.Reason)
		return
	}
...
// notifyAdapterResult posts a notification for an adapter dispatch outcome.
func (b *Brain) notifyAdapterResult(adapter, repo string, issueNum int, status, errMsg string) {
	...
	if err := b.notifier.PostAdapterDispatch(context.Background(), adapter, repo, issueNum, status, errMsg); err != nil {
		b.log.Printf("adapter dispatch notify: %v", err)
	}
}
...
// checkStalledDispatches looks at recent dispatches and warns about failures.
