...
// Store provides shared swarm memory backed by Redis. Keys are flat under a
// single namespace — the squad-era per-scope namespacing was removed in
// octi#271 when the org collapsed to single-tenant.
//
// When a VectorClient and Embedder are configured (via WithVector), Recall
// performs semantic search and merges results with keyword matches. All
// vector operations are best-effort: failures fall back to keyword search
